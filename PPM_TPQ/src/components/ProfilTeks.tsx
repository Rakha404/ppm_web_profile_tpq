...
interface ProfilTeksProps {
  ...
  imageSrc?: string; // Diubah menjadi string untuk menangkap URL backend        
...
export const ProfilTeks: React.FC<ProfilTeksProps> = ({
  ...
  // Memastikan path gambar menunjuk ke server backend lokal jika bukan base64/url luar
  const formattedImage = imageSrc && !imageSrc.startsWith("http") && !imageSrc.startsWith("data:")
  ...
  return (
    <section className="w-full bg-white py-16 px-6 md:px-12 font-sans text-slate-800 overflow-hidden">
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
        ...
        {/* BLOK KONTEN TEKS */}
        <div className="w-full lg:flex-grow lg:w-7/12 space-y-6" data-aos={textAnimation} data-aos-duration="1000">
            ...
            <div className="space-y-3 pt-2">
              ...
              <ul className="space-y-3 pl-5 list-disc text-slate-600 text-sm md:text-base">
                {points.map((point, index) => {
                  ...
                  return (
                    <li key={index} className="leading-relaxed text-justify" data-aos="fade-up" data-aos-delay={pointDelay} data-aos-duration="600" data-aos-anchor-placement="top-bottom">
                      <strong className="text-[#006432] font-bold">{point.title}: </strong>{point.desc}
                    </li>
                  );
                })}
              </ul>

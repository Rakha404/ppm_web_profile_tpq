...
export const Visimisi: React.FC<VisimisiProps> = ({ visi, misi, moto }) => {
  // Fallback data default jika database di MongoDB masih kosong/belum diisi admin
  const fallbackMisi = [
  ...
  const finalMisi = misi && misi.length > 0 ? misi : fallbackMisi;
  const finalMoto = moto && moto.length > 0 ? moto : fallbackMoto;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
          ...
          >
            <div className="space-y-6">
              ...
              <p className="text-slate-600 text-sm leading-relaxed text-justify font-medium italic border-l-4 border-emerald-500 pl-4 py-1">
                "{visi || "Mewujudkan generasi Muslim yang mencintai Al-Qur'an, berakhlak mulia, unggul dalam prestasi keagamaan, serta teguh dalam mengamalkan syariat Islam berlandaskan paham Ahlussunnah wal Jama'ah."}"
              </p>
            </div>
          </div>
              ...
              <div className="flex flex-col gap-3.5">
                {finalMisi.map((misiItem, index) => (
                  <div key={index} className="flex items-start gap-3.5 group/item">
                    ...
                    <p className="text-slate-600 text-sm font-medium leading-relaxed">{misiItem}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          >
            <div className="space-y-6 w-full">
              ...
              <div className="flex flex-col gap-3 justify-center items-center bg-white/80 backdrop-blur-xs rounded-2xl p-6 border border-amber-200/50 shadow-xs">
                {finalMoto.map((motoItem, index) => (
                  <p key={index} className="text-[#006432] text-md md:text-lg font-black italic tracking-wide text-center">
                    "{motoItem}"
                  </p>
                ))}
              </div>
